# --*-- coding : utf-8 --*--
# Project      : python_lemon_作业
# Current file : logger_handler_rizhicaozuo.py
# Author       : 大壮
# Create time  : 2019-10-10 21:01
# IDE          : PyCharm
# TODO 成长很苦，进步很甜，加油！
import os
from lemon_25_191023_充值接口_用例关联_看这里的.test_practice_01_25.setting.constant import p_path

from openpyxl import load_workbook


class ExcelHandler:
    """excel 封装"""

    # 项目来说可能不变
    # 测试数据，写代码之前已经写好测试数据的Excel. 文件名
    def __init__(self, file_name):  # file_name 传文件名
        self.file_name = file_name
        self.wb = load_workbook(file_name)  # 指定的工作簿

    def choose_sheet(self, sheet_name):
        """选择表单.
        sheet_name 是整数，根据索引获取。
        如果是字符串，根据名字获取 '20190920'
        """
        if isinstance(sheet_name, int):  # 工作表
            return self.wb.worksheets[sheet_name]
        return self.wb[sheet_name]

    def headers(self, sheet_name):
        """ 获取标题 """
        sheet = self.choose_sheet(sheet_name)
        # 返回所有的标题头  colmn.value 获取第一行数据的头值
        return [colmn.value for colmn in sheet[1]]

    def read(self, sheet_name, start_row=2, start_column=1):
        """获取所有的数据，字典格式"""
        sheet = self.choose_sheet(sheet_name)
        # TODO 使用列表推导式 获取标题头，1、colmn.value 获取列表中的值
        header = [colmn.value for colmn in sheet[1]]  # 列表中的索引开始是从 1 开始, 而不是 0
        # max_row, max column
        data = []
        for row in range(start_row, sheet.max_row + 1):
            row_data = []
            for column in range(start_column, sheet.max_column + 1):
                row_data.append(sheet.cell(row, column).value)
            # 把标题头和输入组合成字典 把两个列表组合成字典
            row_data = dict(zip(header, row_data))
            data.append(row_data)
        return data

    def save(self):
        """保存"""
        self.wb.save(self.file_name)
        self.wb.close()

    @staticmethod  # 写入表格 静态方法
    def write(file_name, sheet_name, row, column, data):
        wb = load_workbook(file_name)
        sheet = wb.get_sheet_by_name(sheet_name)
        sheet.cell(row, column).value = data
        # 保存关闭
        wb.save(file_name)
        wb.close()


'''
class ExcelHandler2:
    """excel 封装"""
    # 项目来说可能不变
    # 测试数据，写代码之前之前已经写好测试数据的Excel. 文件名
    def __init__(self, file_name):
        self.file_name = file_name
        self.wb = load_workbook(file_name)
        # if isinstance(sheet_name, int):
        #     self.sheet =  self.wb.worksheets[sheet_name]
        # else:
        #     self.sheet = self.wb.get_sheet_by_name(sheet_name)

    def choose_sheet(self, sheet_name):
        """选择表单.
        sheet_name 是整数，根据索引获取。
        如果是字符串，根据名字获取 '20190920'
        """
        if isinstance(sheet_name, int):
            return self.wb.worksheets[sheet_name]
        return self.wb[sheet_name]

    def read_line(self, sheet_name, line):
        """获取行"""
        sheet = self.choose_sheet(sheet_name)
        sheet_data = sheet[line]
        # 元组 （Cell(1,1), Cell(1,2）
        data = []
        for c in sheet_data:
            data.append(c.value)
        return data

    def read(self, sheet_name, start_row=2, start_column=1):
        """获取所有的数据"""
        sheet = self.choose_sheet(sheet_name)
        # max_row, max column
        data = []
        for row in range(start_row, sheet.max_row + 1):
            row_data = []
            for column in range(start_column, sheet.max_column + 1):

                row_data.append(sheet.cell(row, column).value)
            data.append(row_data)
        return data

    def read_cell(self, sheet_name, row, column):
        """一个单元格的数据"""
        sheet = self.choose_sheet(sheet_name)
        return sheet.cell(row, column).value

    def save(self):
        """保存"""
        self.wb.save(self.file_name)
        self.wb.close()

    @staticmethod  # 写入表格
    def write(file_name, sheet_name, row, column, data):
        wb = load_workbook(file_name)
        sheet = wb.get_sheet_by_name(sheet_name)
        sheet.cell(row, column).value = data
        # 保存关闭
        wb.save(file_name)
        wb.close()

'''

if __name__ == '__main__':
    xls = ExcelHandler(os.path.join(p_path.DATA_PATH, 'cases.xlsx'))
    print(xls.read(1))
