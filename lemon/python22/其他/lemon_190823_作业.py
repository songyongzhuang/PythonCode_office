...
print(f'录入数据成功, 您的信息为：{personal_data}')  # 打印字典，展示输入的个人信息  **f占位符
print()
...
print(f'数据添加成功, 您的信息修改为：{personal_data}')
print()


# c, 平台为了保护你的隐私，需要你删除你的联系方式；
input('为了保护您的隐私, 我们需要删除您的联系方式, 请按任意键继续：')  # 这样删除是不有点过分[滑稽]
print(f'您的联系方式：{personal_data.pop("联系方式")}, 删除成功.')  # 根据key值删除元素
...
print('数据添加成功, 您的信息修改为：', personal_data)
...
while True:  # 不确定循环次数, 用True, 一定要防止死循环
    ...
    if key in personal_data.keys():  # 获取字典的key (键)
        ...
        print('信息修改成功：“{}”修改为：{}.\n您的信息为{}\n'.format(key, value, personal_data))
    ...
    if number.isdigit() is True:  # 字符串操作，判断是不是都是正整数
        if int(number) >= 3:  # 判断项目次数至少 3 项
            for i in range(int(number)):  # range() 根据传值循环多少次
                temp = input('请输入要添加的第{}个兴趣：'.format(i))
                i += 1
