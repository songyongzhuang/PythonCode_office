...
class TestLogin:
    ...
    # def test_login_failed_by_wrongData(self, init_driver):
    #     logging.info("******* 登陆功能 - 异常场景用例：数据格式校验 - 用户名为空/密码为空/用户名格式不正确 *******")
    #     # 调用登陆页面的。登陆行为。
    #     init_driver['lp'].login(LD.wrong_datas["user"], LD.wrong_datas["passwd"])
    #     # 断言 - 登陆页面 - 应当现提示信息：请输入手机号
    #     assert (LD.wrong_datas["check"] == init_driver['lp'].get_error_msg())
