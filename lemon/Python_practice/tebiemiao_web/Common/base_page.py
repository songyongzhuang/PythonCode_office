...
import datetime
# 项目路径 截图用
from tebiemiao_web.Common.dir_path import DirPath
# 只处理Windows的上传、下载
import win32gui
import win32con
...
class BasePage(object):
    ...
    def check_element_visible(self, locator, img_doc, timeout=30, poll_frequency=0.5):
        ...
        ****** 检测元素是否存在页面 ******
        ...
        try:
            ...
            logging.exception(f'失败：{timeout}秒内元素{locator}在页面不可见')
            self.save_page_screenshot(img_doc)
            return False
        else:
    ...
    def guendongtiaocaozuo_jianyiban(self, locator, img_doc):
        """
        ****** 滚动条操作简易版 - 不需要预加载的页面 ******
    ...
    def save_page_screenshot(self, img_doc):
        ...
        ****** 网页截图 ******
        ...
        screenshots_path = os.path.join(DirPath.screenshot_dir, f'{img_doc}失败_{now}.png')
        try:
